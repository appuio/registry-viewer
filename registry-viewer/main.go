...
import (
	...
	"github.com/appuio/registry/Godeps/_workspace/src/github.com/abbot/go-http-auth"
	"net/http"
	"os"
)
...
func main() {
  ...
  registry.RemoveEmpty()
	...
	http.HandleFunc("/", authenticator.Wrap(handler))
	http.Handle("/static/", http.StripPrefix("/static/", http.FileServer(http.Dir("/srv"))))
	http.ListenAndServe(":8080", nil)
}

func Secret(user, realm string) string {
	if user == os.Getenv("REGISTRY_VIEWER_USER") {
		d := sha1.New()
		d.Write([]byte(os.Getenv("REGISTRY_VIEWER_PASSWORD")))
